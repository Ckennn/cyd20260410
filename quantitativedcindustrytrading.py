...
def get_board_name(board_code, board_target_df):
    global BOARD_NAME_CACHE
    if board_code in BOARD_NAME_CACHE:
        return BOARD_NAME_CACHE[board_code]

    board_name = ''
    if dfutil.not_empty(board_target_df):
        # board_code_key = 'board_code'
        df_board_code = board_target_df[board_target_df[qldef.board_code_key] == board_code]
        if dfutil.not_empty(df_board_code):
            ...
            board_name = df_board_code.iloc[0][qldef.board_name_key]

    if board_name:
        BOARD_NAME_CACHE[board_code] = board_name
    return board_name
...
def get_board_df(board_code, board_target_df):
    ...
    if dfutil.not_empty(board_target_df):
        # board_code_key = 'board_code'
        df_result = board_target_df[(board_target_df[qldef.board_code_key] == board_code)
...
def get_industry_daily_quote_data(board_code, start_date: int, end_date: int, filter_str="1d_ind.csv"):
    # 假设东财行业数据存储在个股行情目录中 (或需要指定其他目录)
    # 如果数据不存在，get_code_daily_quote_data 会返回 None
    target_path = qldef.market_quotation_directory 
    # 禁用fallback遍历，提高性能 (allow_fallback=False)
    df = qloption.database.get_code_daily_quote_data(board_code, start_date, end_date, target_path, filter_str, allow_fallback=False)
    return df
...
def get_tobe_traded_stocks(date: datetime):
    ...
    date_str = dfutil.date_by_datetime(date)
    filename = f"stocks_tobe_traded_{date_str}.csv"
    file_path = os.path.join(cache_dir, filename)
    
    if os.path.exists(file_path):
        return qloption.database.read_single_big_csv(file_path)
    return None
...
def get_trade_state(date: int, df_all, board_target_df, board_code, is_buy: bool = True) -> [bool, pd.DataFrame]:
    ...
    if dfutil.not_empty(df_all):
        ...
        if dfutil.not_empty(df):
            if str(date) == '20240222' and board_code == 'BK0465':
                logutil.log.critical(f"DEBUG: Checking trade for {date} {board_code}")
                logutil.log.critical(f"DEBUG: df size: {len(df)}")

            if is_buy:
                # 买入：判断某列是否包含特定的字符串数据 str.contains(board_code)
                df_date_trade = df[df[trade_type_key] == qldef.trade_buy_type]
                if str(date) == '20240222' and board_code == 'BK0465':
                     logutil.log.critical(f"DEBUG: df_date_trade size: {len(df_date_trade) if df_date_trade is not None else 0}")
                     if dfutil.not_empty(df_date_trade):
                         logutil.log.critical(f"DEBUG: df_date_trade columns: {df_date_trade.columns}")
                         logutil.log.critical(f"DEBUG: df_date_trade head: \n{df_date_trade.head().to_string()}")
                         val = df_date_trade.iloc[0].get(qldef.board_code_key)
                         logutil.log.critical(f"DEBUG: board_code_key val: {val} (type: {type(val)})")
                         logutil.log.critical(f"DEBUG: target board_code: {board_code} (type: {type(board_code)})")
                         
                if dfutil.not_empty(df_date_trade):
                    # 使用 board_code 和 board_name
                    df_date_code = df_date_trade[(df_date_trade[qldef.board_code_key] == board_code)
                                                 | (df_date_trade[qldef.board_name_key] == board_name)
                                                 | (df_date_trade[qldef.board_name_key].str.contains(board_name))]
                    
                    if str(date) == '20240222' and board_code == 'BK0465':
                        logutil.log.critical(f"DEBUG: df_date_code size: {len(df_date_code)}")
...
if bt is not None:
    ...
    class my_strategy(bt.Strategy):
        ...
        board_target_df = qloption.database.get_board_target_df()  # 东方财富行业板块数据
                ...
                if is_buy:
                    if all_cash > 0:
                        ...
                        if is_can_trade:
                            ...
                            # 优化：动态仓位管理 (百分比仓位)
                            # 假设我们要构建一个分散的组合，比如最多持仓 5 只行业 ETF
                            target_pos_count = 5 
                            # 单只标的的最大仓位比例 = 1 / 5 = 20%
                            # 但为了保守起见，或者留有现金，我们设置单只最大仓位为总资金的 18%
                            # 且受到当前可用现金的限制
                            
                            total_value = self.broker.getvalue() # 总资产 (现金+持仓)
                            target_value = total_value * 0.18    # 目标单只持仓市值
                            
                            # 实际下单金额取：min(目标金额, 当前可用现金)
                            order_value = min(target_value, all_cash)
                            
                            if order_value < 5000: # 资金太少就不交易了
                                self.log(f"资金不足: {board_code} 需 {target_value:.2f}, 仅有 {all_cash:.2f}, 跳过", is_save_log=True)
                                return
                            ...
                            if close > 0:
                                ...
                                if order_amount > 0:
                                    ...
                                    self.log(f"买入下单: {board_code} 金额={order_value:.2f} 数量={order_amount}", is_save_log=True)
                            else:
                                self.log(f"警告: {board_code} 收盘价为 {close}，跳过买入", is_save_log=True)
        ...
        def notify_order(self, order):
            board_code = order.data.params.dataname[qldef.board_code_key].iloc[0]
            trade_type = '买入'
        ...
        def prenext(self):
            ...
            1、这只股票的开始日，未必是另一只股票的开始日，这回导致另一只股票没数据
        ...
        def next(self):
            ...
            while i < len(self.datas):
                ...
                board_code = data.params.dataname[qldef.board_code_key].iloc[0]

                current_date_int = dfutil.date_by_datetime(current_datetime)
                
                if str(current_date_int) == '20240222':
                     if str(board_code) == 'BK0465':
                         logutil.log.critical(f"DEBUG: In next() for 20240222, found BK0465. Type: {type(board_code)}")
                     elif 'BK0465' in str(board_code):
                         logutil.log.critical(f"DEBUG: In next() for 20240222, found partial BK0465: {board_code}")
...
def run_strategy(start_date: int, end_date: int, process_lock=None):
    ...
    # 使用多线程加载
    max_workers = min(32, len(date_list)) if len(date_list) > 0 else 1
    ...
    board_code_list = get_dc_industry_code_list()
    logutil.log.critical(f"DEBUG: Total board codes: {len(board_code_list)}")
    if 'BK0465' in board_code_list:
        logutil.log.critical("DEBUG: BK0465 is in board_code_list")
    else:
        logutil.log.critical("DEBUG: BK0465 is NOT in board_code_list")
    ...
    # 性能优化：并行加载行业数据
    # ---------------------------------------------------------
    logutil.log.info(f"正在并行加载 {len(board_code_list)} 个行业板块数据...")
    ...
    try:
        ...
        pbar = tqdm(total=len(board_code_list), desc="加载行业数据", unit="个")
    except ImportError:
        pbar = None
    
    def load_industry_data(board_code):
        # 获取数据
        df = get_industry_daily_quote_data(board_code, start_date, end_date)
        if board_code == 'BK0465':
             if dfutil.not_empty(df):
                 logutil.log.critical(f"DEBUG: BK0465 loaded. Range: {df.index[0]} to {df.index[-1]}. Rows: {len(df)}")
             else:
                 logutil.log.critical("DEBUG: BK0465 loaded but EMPTY.")
        return board_code, df

    with ThreadPoolExecutor(max_workers=min(32, len(board_code_list)) if len(board_code_list) > 0 else 1) as executor:
        for board_code, board_df in executor.map(load_industry_data, board_code_list):
            if pbar:
                pbar.update(1)
            if dfutil.not_empty(board_df):
                loaded_datas.append((board_code, board_df))

    if pbar:
        pbar.close()

    logutil.log.info(f"行业数据加载完成，耗时: {time.time() - load_start_time:.2f}秒，有效数据: {len(loaded_datas)}条")

    for board_code, board_df in loaded_datas:
        ...
        for col in numeric_cols:
            if col in board_df.columns:
                board_df[col] = pd.to_numeric(board_df[col], errors='coerce').fillna(0.0)
        
        # Ensure board_code column exists
        if qldef.board_code_key not in board_df.columns:
            board_df[qldef.board_code_key] = board_code
